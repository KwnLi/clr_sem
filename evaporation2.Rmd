...
for(i in 1:n.rep){
  ...
  model.i <- lmer(log(evapRate_ng) ~ poly(decTime, 2) + (poly(decTime, 1)|dateFac) + (1|locID),
          data = data.i, 
          REML = F)
  ...
  new.i$pred = exp(predict(model.i, newdata = new.i, re.form = ~ (1|locID), allow.new.levels = TRUE))
  
  predict.i <- exp(predict(model.i))
  out.predict <- exp(predict(model.i, newdata = data.out, allow.new.levels = TRUE))
  
  crossvaldat[[i]] <- bind_rows(data.i %>% select(locID, decTime, dateFac, evapRate_ng) %>% 
                                  mutate(predict = predict.i) %>% mutate(cross.class = "train"),
                                data.out %>% select(locID, decTime, dateFac, evapRate_ng) %>% 
                                  mutate(predict = out.predict) %>% mutate(cross.class = "out")) %>%
    mutate(i = i)
  
  crosspred[[i]] <- new.i %>% select(locID, pred) %>% pivot_wider(names_from = locID, values_from = pred)
  
  crossmods[[i]] <- model.i
}

preduncert <- bind_rows(crosspred) %>% rownames_to_column(var = "sim") %>% mutate(sim = as.numeric(sim)) %>%
  pivot_longer(cols = -sim) %>%
  rename(locID = name, pred = value)

ggplot(preduncert, aes(x = pred)) + geom_histogram(fill = "red", alpha = 0.2) + facet_wrap(~locID) + 
  geom_vline(data = pred_10AM, mapping = aes(xintercept = pred.evap.1)) +
  theme_article()

pred_sum <- preduncert %>% group_by(locID) %>%
  summarize(mn = mean(pred, na.rm = T), sd = sd(pred, na.rm = T)) %>%
  left_join(evap_all %>% select(locID, locType) %>% distinct())

ggplot(pred_sum, aes(locID, mn)) + geom_pointrange(aes(ymin = mn - sd, ymax = mn + sd, color = locType)) + 
